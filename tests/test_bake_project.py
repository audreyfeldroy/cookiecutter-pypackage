...
def test_bake_not_open_source(cookies):
    with bake_in_temp_dir(cookies, extra_context={'open_source_license': 'Proprietary'}) as result:
        found_toplevel_files = [f.basename for f in result.project.listdir()]
        assert 'setup.py' in found_toplevel_files
        assert 'Free Software:' not in result.project.join('README.rst').read()
        assert 'Copyright (c)' in result.project.join('README.rst').read()
        assert 'LICENSE' in found_toplevel_files
        assert 'Copyright (c)' in result.project.join('LICENSE').read()
