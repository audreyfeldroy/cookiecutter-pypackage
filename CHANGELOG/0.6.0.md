# Cookiecutter PyPackage 0.6.0: Clearer names and safer GitHub setup

WriterStead needed a new set of Air packages, and those packages needed a
better foundation. That brought me back to Cookiecutter PyPackage: before I
could release the new Air packages, I needed to release the template they
would be built from.
...
- **Trust the reported package version.**
  `cookiecutter_pypackage.__version__` reads the installed distribution
  metadata, keeping runtime and release versions aligned.
